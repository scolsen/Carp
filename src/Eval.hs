...
primitiveDefmodule xobj ctx@(Context env i tenv pathStrings _ _ _ _) (XObj (Sym (SymPath [] moduleName) _) _ _ : innerExpressions) =
  ...
  either (const (defineNewModule emptyMeta)) updateExistingModule (E.lookupBinder ((fromRight env (E.getInnerEnv env pathStrings)) {envParent = Nothing}) (SymPath [] moduleName))
    ...
    >>= \(newCtx, result) ->
      let updater c = (c {contextInternalEnv = (E.parent =<< contextInternalEnv c),
                          contextTypeEnv = E.progenitor (contextTypeEnv c)})
       in case result of
            Left err -> pure (newCtx, Left err)
  ...
  where
    ...
    updateExistingModule (Binder _ (XObj (Mod innerEnv mTypeEnv) _ _)) =
      let updateContext = replacePath' (contextPath ctx ++ [moduleName])
                        . replaceTypeEnv' (E.setParent mTypeEnv tenv)
    ...
    defineNewModule meta =
      ...
      where
        ...
        updater     = \c -> insertInGlobalEnv' (markQualified (SymPath pathStrings moduleName)) (Binder meta newModule) c
                    ...
                    -- >>= pure . replaceTypeEnv' moduleTypes
