...
--next b@(Binder _ (XObj (Mod ev _) _ _)) = Right (ev, b)
--next _ = Left NoEnvInNonModule
...
findChildren e =
  ...
  --foldl' getEnv [] (Map.elems (binders e))
...
lookupChildren e = 
  ...
  where go acc e' = case findChildren e' of 
                     [] -> (e':acc)
                     xs -> (foldl' go [] xs ++ acc)
  --(findChildren e) ++ concatMap lookupChildren (findChildren e) 
...
findImportedEnvs e =
  let usePaths = Set.toList (envUseModules (prj e))
   ...
   in (rights used) --(fmap fst (rights used))
...
lookupExhuastive f e name =
  let --binds  = Map.toList (binders e)
      envs   = [e] ++ (f e)
      --vals   = fmap (\e' -> (fmap (e',) (find e' name))) envs
      --spine  = rights [(fmap (\b -> (e,b)) (find e name))]
      --leaves = rights (fmap (\e' -> fmap (\b -> (e',b)) (find e' name)) (f e))
      --top    = maybe [] (\e' -> lookupExhuastive f e' name) (parent e)
   in (go (parent e) envs)
   where go _ [] = []
         go Nothing xs = foldl' accum [] xs  --fmap (\e' -> (fmap (e',) (find e' name))) xs
         --go Nothing xs = fmap (\e' -> (fmap (e',) (find e' name))) xs
...
lookupInUsed e global spath =
  ...
  where go :: Environment e => [(e, Binder)] -> e -> [(e, Binder)] 
        go acc e' = case (Env.lookup e' spath) of
                      Right (e'', b) -> ((e'', b):acc)
                      _ -> acc
    --rights (map (\e' -> fmap (\b -> (e', b)) (find e' name)) (findImportedEnvs e))
...
-- | Lookup a binder in all the used environments in a given environment.
--lookupBinderInImports :: Environment e => e -> String -> [Binder]
--lookupBinderInImports = lookupBinderExhuastive findImportedEnvs
...
-- | Lookup a binder in an environment, all its used environments and its parent.
--lookupBinderContextually :: Environment e => e -> SymPath -> Either EnvironmentError [Binder]
--lookupBinderContextually e (SymPath [] name) = 
--  case lookupBinderInImports e name of 
--    [] -> Left (BindingNotFound name (prj e))
--    xs -> Right xs
--lookupBinderContextually e path@(SymPath (p : ps) name) =
--  lookupDirectly <> lookupInUsedAndParent
--  where lookupDirectly = (next e p)
--                           >>= \(e', _) -> lookupBinder e' (SymPath ps name)
--                           >>= pure . (:[])
--        lookupInUsedAndParent = case rights (fmap ((flip lookupBinder) path) (findImportedEnvs e)) of 
--                                  [] -> Left (BindingNotFound name (prj e))
--                                  xs -> 
--                                    case parent e of 
--                                      Nothing -> Right xs
--                                      Just e' -> (lookupBinder e' path >>= \found -> Right $ xs ++ [found]) <> Right xs
...
--concatMap select (binders e)
--  where
--    select :: Binder -> [String]
--    select binder =
--      case inj (next binder) of 
--        Left _ -> [getName (binderXObj binder)]
--        Right (e',_) -> envBindingNames e'
...
findAllGlobalVariables e =
  --concatMap finder (Map.toList (binders e))
  ...
  where
    ...
    finder acc _ = acc
    --  case (next e name) of 
    --    Left _ -> getVar binder
    --    Right (e',_) -> findAllGlobalVariables e'
    --getVar :: Binder -> [Binder]
    --getVar def@(Binder _ (XObj (Lst (XObj Def _ _ : _)) _ _)) = [def]
    --getVar _ = []
