...
typeEnvToDeclarations typeEnv global =
  let -- We need to carry the type environment to pass the correct environment on the binderToDeclaration call.
      addEnvToScore tyE =  
        map (\(score, binder) -> (tyE, score, binder)) (sortDeclarationBinders tyE (map snd (Map.toList (binders tyE))))
      bindersWithScore = trace (show "adding: " ++ (show ((map fst . Map.toList) (binders typeEnv)))) (addEnvToScore typeEnv)
      mods = (trace ("Mods: " ++ show (findModules global)) (findModules global))
      folder = (\sorted (XObj (Mod e t) _ _) ->
                    trace (show "adding: " ++ show (envModuleName e) ++ (show ((map fst . Map.toList) (binders t))))
                    sorted ++ (foldl folder (addEnvToScore t) (findModules e)))
      allScoredBinders = sortOn (\(_, i, _) -> i) (foldl folder bindersWithScore mods)
   in do
        okDecls <-
          mapM
            ( \(te, score, binder) ->
                fmap
                  (\s -> if s == "" then "" else ("\n// Depth " ++ show score ++ "\n") ++ s)
                  (traceShowId (binderToDeclaration te binder))
            )
            allScoredBinders -- (traceShowId allScoredBinders)
        pure (concat okDecls)
