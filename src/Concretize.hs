...
import Debug.Trace
import Env (insertX, insertInPlace, lookupBinder, envIsExternal, lookup, lookupEverywhere, findPoly, findAllByName)
import Info
import Managed
        ...
        case visitedBody of
          Right okBody ->
            let -- Analyse the body of the lambda to find what variables it captures
                ...
                tyPath = (SymPath spath environmentTypeName)
                extendedArgs =
                ...
                extendedTypeEnv = trace (show typeEnv) replaceLeft (FailedToAddLambdaStructToTyEnv tyPath environmentStruct) (insertInPlace typeEnv environmentTypeName (toBinder environmentStruct))
...
concretizeType typeEnv genericStructTy@(StructTy (ConcreteNameTy spath) _) =
  case trace ("The spath: " ++ show spath ++ "::" ++ show (envModuleName (getTypeEnv typeEnv))) (lookup typeEnv spath) of
