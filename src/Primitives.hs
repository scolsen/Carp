  ...
  where
    ...
    ensureUnqualified objs =
      if all isUnqualifiedSym objs
        then deftype ctx name (selectConstructor (contextTypeEnv ctx) rest)
        else pure (toEvalError ctx xobj (QualifiedTypeMember rest))
primitiveDeftype _ ctx (name : rest) =
  deftype ctx name (selectConstructor (contextTypeEnv ctx) rest)
...
selectConstructor :: TypeEnv -> [XObj] -> (Ty -> (XObj, [XObj], ModuleCreator))
selectConstructor tenv xs =
  let 
      (constructor, creator, mems) = if length xs == 1 && isArray (head xs)
                                       then (Deftype, moduleForDeftypeInContext, (qualifyLocalMembers tenv xs))
                                       else (DefSumtype, moduleForSumtypeInContext, (qualifyLocalMembers tenv xs)) 
        ...
        updateModuleUsePaths e p (Binder meta (XObj (Mod ev et) i t)) = 
          either 
            ...
            ((unwrapErr (insert e p (Binder meta (XObj (Mod (addUsePath ev path) et) i t))))
              ...
              -- >>= \ctx' -> pure (maybe ctx' (\i -> replaceInternalEnv ctx' (addUsePath i path)) (contextInternalEnv ctx')))
        updateModuleUsePaths _ _ _ =
          (evalError ctx "Context path pointed to non-module!" (xobjInfo xobj))
-- ((unwrapErr (lookupBinder (contextGlobalEnv ctx) name))
--             >>= (\b -> case b of 
--                         (Binder _ (XObj (Mod _ _) _ _)) -> 
--                           pure . (replaceGlobalEnv ctx ((contextGlobalEnv ctx) {envUseModules = Set.insert path (envUseModules (contextGlobalEnv ctx))}))
--                         _ -> Left "Paths doesn't denote a module"))
--primitiveUse xobj ctx (XObj (Sym path _) _ _) =
--  pure $ either (\err -> (evalError ctx err (xobjInfo xobj)))
--                (\updatedCtx -> (updatedCtx, dynamicNil))
--                look
--  where
--    pathStrings = contextPath ctx
--    global = contextGlobalEnv ctx
--    look = ((unwrapErr (lookupBinder global pathStrings))
--             >>= (\b -> case b of 
--                         (Binder meta (XObj (Mod ev et) i t)) -> 
--                           Right ev >>= \e' -> (unwrapErr (lookupBinder e' path) <> (unwrapErr (lookupBinder global path)))
--                                    >>= \b' -> addUsePath e' path b' b
--                         _ -> Left "Pathstrings don't denote a module"))
--    -- | Add path to the list of used modules for the environment.
--    addUsePath :: Env -> SymPath -> Binder -> Either String Context
--    addUsePath e path (Binder _ (XObj (Mod _ _) _ _)) (Binder meta (XObj (Mod ev et) i t)) = 
--      let e' = (e {envUseModules = Set.insert path (envUseModules e)})
--       in ((unwrapErr (replace global pathStrings (Binder meta (Mod e' et) i t)))
--          >>= pure . (replaceGlobalEnv ctx))
--    addUsePath e path _ _ = Left ("Can't find a module named '" ++ show path ++ "'") 
