...
import Context
import Data.Maybe
import Env (new, addListOfBindings, next, lookup)
import Info
import Managed
import Obj
import StructUtils
...
moduleForDeftype innerEnv typeEnv env pathStrings typeName typeVariables rest i existingEnv =
  let moduleValueEnv = fromMaybe (new innerEnv (Just typeName)) existingEnv
      ...
      --qualifiedMembers = qualifyLocalMembers typeEnv rest
   in do
...
-- |
qualifyLocalMembers :: TypeEnv -> [XObj] -> [XObj]
qualifyLocalMembers tenv xs =
  case xs of
    [XObj (Arr members) info ty] ->
      [XObj (Arr (unTuple (map (second go) (pairwise members)) [])) info ty]
    _ -> map qualifySum xs 
  where qualifySum :: XObj -> XObj
        qualifySum (XObj (Lst [h, (XObj (Arr mems) ii tt)] ) i t) =
           (XObj (Lst [h, (XObj (Arr (map go mems)) ii tt)] ) i t)
        qualifySum x = x
        go :: XObj -> XObj
        go x@(XObj (Sym path mode) i t) =
          case Env.lookup tenv path of 
            Right (e, _) -> 
              case envModuleName (getTypeEnv e) of 
                Just modName ->
                  (XObj (Sym (consPath [modName] path) mode) i t)  
                _ -> x
            _ -> x
        go x = x 
        unTuple :: [(XObj, XObj)] -> [XObj] -> [XObj]
        unTuple [] acc = acc
        unTuple (y:ys) acc = unTuple ys (acc ++ ((fst y):(snd y):[]))
--qualifyLocalMembers tenv [XObj (Arr members) info ty] = 
--  unTuple (map (second go) (pairwise members)) []
--  where go :: XObj -> XObj
--        go x@(XObj (Sym path mode) i t) =
--          case Env.lookup tenv path of 
--            Right (e, _) -> 
...
--        go x = x 
--        unTuple :: [(XObj, XObj)] -> [XObj] -> [XObj]
--        unTuple [] acc = [XObj (Arr acc) info ty]
--        unTuple (x:xs) acc = unTuple xs (acc ++ ((fst x):(snd x):[]))
--qualifyLocalMembers tenv xs = map go xs
--  where go :: XObj -> XObj
--        go (XObj (Lst [h, (XObj (Arr members) ii tt)]) i t) = 
--qualifyLocalMembers _ xs = error ("unreachable" ++ joinWithComma (map pretty xs))
